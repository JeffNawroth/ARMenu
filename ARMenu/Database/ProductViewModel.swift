    ...
    func getData(){
        ...
        db.collection("Products").addSnapshotListener { snap, err in
            ...
            for i in snap!.documentChanges{
                ...
                self.products.append(Product(id: id, image: image, name: name, category: category, price: price, description: description, isVegan: isVegan, isBio: isBio, isFairtrade: isFairtrade, nutritionFacts: nutritionFacts, allergens: allergens, additives: additives))
            }
        }
    }
    ...
    func fetchData(){
        ...
        db.collection("Products").addSnapshotListener { querySnapshot, error in
            ...
            self.products = documents.map({ queryDocumentSnapshot in
                ...
                let nutritionFacts = data["nutritionFacts"] as? [Double] ?? [0]
                let allergens = data["allergens"] as? [String] ?? []
                let additives = data["additives"] as? [String] ?? []

                return Product(id: queryDocumentSnapshot.documentID,image: image, name: name, category: category, price: price, description: description, isVegan: isVegan, isBio: isBio, isFairtrade: isFairtrade, nutritionFacts: nutritionFacts, allergens: allergens, additives: additives)
